<?php
    ...
    if (isset($_GET['aksi'])) {
        if ($_GET['aksi'] == 'hapus') {
            ...
            // Delete the dependent records from the jadwal_periksa table
            $deleteJadwalPeriksa = mysqli_query($mysqli, "DELETE FROM jadwal_periksa WHERE id_dokter = '$id'");
    
            // If the dependent records are successfully deleted, delete the record from the dokter table
            if ($deleteJadwalPeriksa) {
...
<main id="jadwaldokter-page">
    ...
    <select class="form-select" name="id_dokter" aria-label="id_dokter" onchange="changeDoctor(this.value)">
...
<script>
    function changeDoctor(id) {
        location.href = 'index.php?page=jadwalDokter&id=' + id;
    }
</script>
                    <div class="dropdown mb-3 w-25">
